...
class PandaSet(torch.utils.data.Dataset):
    def __init__(self,
        ...
        dataset = pandaset.DataSet(data_path)
        self.seq_object = dataset[self.seq].load()
        self.scans = [self.seq_object.lidar[i].to_numpy() for i in range(80)]  # every sequence has 80 scans
    ...
    def get_single_sample(self, index, vote_idx=0):
        ...
        points = scan[:, :4]
        org_points = scan[:, :4].copy()
        labels_in = np.zeros(points.shape[0]).astype(np.uint8)
        points[:, :3] /= 8
        # Scale the last column between 0 and 1
        min_val = np.min(points[:, 3])  # Minimum value of the last column
        max_val = np.max(points[:, 3])  # Maximum value of the last column
        points[:, 3] = (points[:, 3] - min_val) / (max_val - min_val)  # Scale the last column
        ...
        coords, xyz, feats, labels, inds_reconstruct = data_prepare(xyz, feats, labels_in,
                                                                        ...
                                                                        self.xyz_norm)
        # print("coords idx = ", len(inds_reconstruct))
        # print('length of after data prep =', len(xyz))
